...
02_ss_oligonucleotide.xlsx

InputFile
---------
02_ss_oligonucleotide.xlsx : 計算管理ファイル 必須： id_sequence, name, sequence
...
import pandas as pd
from oligomer import Base, Sugar, Oligomer, ConstantsForSequence, Sequence, SequenceForOP
...
def stop_watch(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        print(f">> End     : {func.__name__:^16} | ", f"RAP   : {minitues:02d}′{sec:02d}″{msec:}")
        return result
...
class SequenceForRena(Sequence):
    ...
    def check_sugar_and_convert_base(
        sugar_tobe_checked: Sugar, base_tobe_checked: Base, constants: ConstantsForSequence
    ) -> Base:
        """
        ...
        if should_base_be_mecytosine(sugar_tobe_checked, base_tobe_checked, constants):
            checked_base = constants.dict_bases["mC"]
        elif should_base_be_uracil(sugar_tobe_checked, base_tobe_checked, constants):
    ...
    def create_fm_baseline_as_gapmer(
        ...
        if sum(wing) < len(sequence_baseline):
            ...
            oligomer.sequence_table["linker"][wing[0] : -wing[1] - 1] = obj_link_gap
            ...
            for i_mer in oligomer.sequence_table.index:
                ...
                checked_base = cls.check_sugar_and_convert_base(i_sugar, i_base, constants)
                oligomer.sequence_table.loc[i_mer, "base"] = checked_base
            obj_sequence = cls.create_fm_sequence_table(oligomer.sequence_table, constants)
        return obj_sequence
    ...
    def is_complementary(
        ...
        complementary_baseline = self.get_complementary_baseline(rna=True, reverse=True)
        ...
        if print_checksheet:
            ...
            baseline_comprementary = complementary_sequence.get_baseline(rna=True, reverse=True)
...
class RenaOligomer(Oligomer):
    ...
    def read_renafile(
        ...
        **kwargs,
        ...
        """Rena管理ファイルからの読み込み."""
        sequence = SequenceForRena.create_fm_sequence_ns(sequence_ns=sequence, constants=constants)
        flp = cls(
            name=name,
            note=memo,
            sequence=sequence,
...
class BatchSheet:
    ...
    def calc_flp(self):
        ...
        self.data_for_calc_flp["object"] = self.data_for_calc_flp.apply(
            lambda x_flp_info: RenaOligomer.read_renafile(constants=self.constants, **x_flp_info),
            axis=1,
        )
    ...
    def output_flp_summary(self):
        ...
        self.summary_flp = self.data_for_calc_flp.apply(
            lambda x_flp_df: x_flp_df["object"].expand_for_summary(), axis=1, result_type="expand"
        )
        dirname = rf"{DIR_NAME.parent}/データベース/data"
        self.summary_flp.to_csv(
            rf"{dirname}/02_ss_oligonucleotide.csv", index=False, encoding="utf-8-sig"
        )
    ...
    def get_obj(self, id_sequence):
        obj = self.data_for_calc_flp[self.data_for_calc_flp["id_sequence"] == id_sequence]["object"]
        return list(obj)[0]
...
if __name__ == "__main__":
    ...
    sheet = BatchSheet(FILENAME_SEQUENCE, const)
    sheet.calc_all()
    os.startfile(rf"{DIR_NAME.parent}/データベース/02_ss_oligonucleotide.xlsx")
